...
export default function InventoryAudit() {
  ...
  const fetchAdjustments = async () => {
    try {
      ...
      if (data.success) {
        const mapped = data.data.map((sa: any) => {
  ...
  const isSessionAdjusted = (s: InventorySession | null) => {
    if (!s) return false;
    if (s.status === 'COMPLETED') return true;
    // If active session is inspected, check if any items are in adjustments
    if (activeSession && activeSession.id === s.id && sessionItems.length > 0) {
      const hasAdjustedItem = sessionItems.some(item =>
        adjustments.some(adj => adj.inventory_item_id === item.id)
      );
      if (hasAdjustedItem) return true;
    }
    // Check if any adjustment note specifies this session
    return adjustments.some(adj =>
      adj.note && adj.note.includes(`Session #${s.id}`)
    );
  };
  ...
  const startNewSession = async () => {
    try {
      ...
      if (data.success) {
        ...
        alert(data.message || 'Failed to start audit session.');
        setLoading(false);
      }
    } catch (err) {
      console.error(err);
      alert('Audit service is currently unavailable.');
      setLoading(false);
    }
  };
  ...
  const handleProductSelect = (p: Product) => {
    if (isSessionAdjusted(activeSession)) {
      alert("The session has already been adjusted, and can not be edited or deleted.");
      return;
    }
        ...
        <div className="h-full flex flex-col bg-slate-50">
          ...
          <div className="p-4 bg-white border-b border-slate-200 flex items-center justify-between shadow-sm sticky top-0 z-50">
             <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                     ...
                     <h1 className="font-black text-slate-900 tracking-widest uppercase">Audit Session #{activeSession.id}</h1>
                     {isSessionAdjusted(activeSession) ? (
                       <span className="px-1.5 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 text-[8px] font-black uppercase tracking-widest leading-none">Adjusted</span>
                     ) : (
                       <span className="px-1.5 py-0.5 rounded bg-amber-500/10 border border-amber-500/20 text-amber-600 text-[8px] font-black uppercase tracking-widest leading-none">Draft</span>
                     )}
                  </div>
                  <div className="text-[8px] text-slate-400 font-bold uppercase tracking-widest mt-0.5 mt-1">Started on: {new Date(activeSession.created_at).toLocaleString()}</div>
                </div>
             </div>
             ...
             <div className="flex items-center gap-2">
                ...
                >
                  <RefreshCcw className={`w-3.5 h-3.5 ${completing ? 'animate-spin' : ''}`} /> Sync Stock Levels
                </button>
             </div>
          </div>
                   ...
                   <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50 gap-4 flex-wrap">
                    <div className="flex items-center gap-4 flex-1 min-w-[300px]">
                        <div className="relative flex-1">
                           ...
                           <input 
                             type="text"
                             placeholder="Scan barcode or search product to count..."
                    ...
                    {selectedProduct && (
                      <div className="flex items-center gap-2 bg-indigo-600 p-1 rounded shadow-lg animate-in fade-in slide-in-from-right-4 duration-300">
                        <div className="px-2">
                           <div className="text-[8px] font-black text-indigo-100 uppercase leading-none mb-1">Selected:</div>
                   ...
                   <table className="w-full text-left">
                      <thead className="sticky top-0 bg-slate-50 z-10 border-b border-slate-200">
                         <tr className="text-[7px] font-black text-slate-400 uppercase tracking-widest">
                            <th className="px-6 py-3">Product Name & Barcode</th>
                            ...
                            <th className="px-6 py-3 text-right">Value Difference</th>
    ...
    <div className="h-full flex flex-col bg-slate-50 overflow-hidden">
      ...
      <div className="p-4 bg-white border-b border-slate-200 flex items-center justify-between shadow-sm">
         <div className="flex items-center gap-3">
             <ClipboardCheck className="w-4 h-4 text-indigo-600" />
             <h1 className="font-black text-slate-900 tracking-widest uppercase">Stock Audits</h1>
         </div>
         ...
         {stats && (
           <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm border-l-4 border-l-indigo-600">
                 <div className="text-[7px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">Total Sessions</div>
              ...
              <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm border-l-4 border-l-red-500">
                 <div className="text-[7px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">Total Stock Loss</div>
              ...
              <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm border-l-4 border-l-emerald-500">
                 <div className="text-[7px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">Total Stock Gain</div>
              ...
              <div className="bg-slate-900 p-5 rounded-xl shadow-2xl border border-white/5">
                 <div className="text-[7px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">Net Value Difference</div>
         ...
         <div className="max-w-7xl mx-auto space-y-4 w-full">
            ...
            <div className="space-y-4">
               <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="text-[9px] font-black text-slate-500 uppercase tracking-[0.3em]">Audit Sessions</h3>
               </div>
                  ...
                  <table className="w-full text-left">
                     ...
                     <tbody className="divide-y divide-slate-100">
                        {loading && sessions.length === 0 ? (
                           <tr>
                              <td colSpan={6} className="py-20 text-center animate-pulse tracking-[0.5em] text-indigo-600 font-black">Loading audit history...</td>
                           </tr>
                        ) : sessions.length === 0 ? (
                           <tr>
                              <td colSpan={6} className="py-20 text-center text-slate-400 font-black uppercase tracking-[0.5em]">No audit sessions found</td>
                           </tr>
                        ) : sessions.map((s, i) => (
                           <tr key={s.id} className="hover:bg-slate-50 transition-colors group">
                              ...
                              <td className="px-6 py-4 text-center">
                                 <span className={`px-2 py-0.5 rounded text-[7px] font-black uppercase tracking-widest ${
                                    ...
                                    {isSessionAdjusted(s) ? 'ADJUSTED' : 'DRAFT'}
                                 </span>
                              </td>
                     ...
                     <tfoot className="bg-slate-50 border-t border-slate-200">
                        <tr className="text-[8px] font-black text-slate-900 uppercase tracking-widest">
                          <td className="px-6 py-4">Total Difference</td>
      ...
      <ConfirmModal 
        ...
        message={`Are you sure you want to delete Audit Session #${sessionToDelete}? This action cannot be undone.`}
      />
      ...
      <ConfirmModal 
        ...
        onConfirm={confirmDeleteItem}
        title="Remove Item"
        message="Are you sure you want to remove this audited item from the session?"
      />
    </div>
  );
}
